...
function webinarignition_store_phone_callback() {
	...
	$sql = "SELECT * FROM {$table_db_name} WHERE id = '{$ID}'";
	$lead = $wpdb->get_row($sql, OBJECT);

	if (empty($lead)) {
		$sql = "SELECT * FROM {$table_db_name} WHERE hash_ID = '{$ID}'";
		$lead = $wpdb->get_row($sql, OBJECT);
	}
...
function webinarignition_broadcast_msg_poll_callback() {
	...
	$lookUpIP      = $wpdb->get_row( "SELECT * FROM $table_db_name WHERE app_id = '$ID' AND ip = '$IP'  AND lead_id = '$LEAD_ID' ", OBJECT );
...
function webinarignition_hold_or_release_console_question() {
	...
	$questions = $wpdb->get_results("SELECT * FROM $table_db_name WHERE attr2 = '$supportId'", ARRAY_A);
...
function webinarignition_release_unanswered_questions() {
	...
	$questions = $wpdb->get_results("SELECT * FROM $table_db_name WHERE app_id = '$webinarId' AND attr2 = '$supportId'", ARRAY_A);
