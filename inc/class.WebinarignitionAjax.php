...
class WebinarignitionAjax {
	public static function add_lead_check_secure() {
		...
		if (
			...
			self::error_response(['message' => __('Error', 'wp2leads_itm_mid') . ': ' . __('Cheating, huh!!!.', 'webinarignition')]);
		}
		...
		if (!WebinarignitionPowerups::is_secure_access_enabled($webinar_data)) {
			self::success_response(['message' => __('Success', 'wp2leads_itm_mid') . ': ' . __('Great lets register.', 'webinarignition')]);
		}
		...
		if (!empty($secure_access_webinar_blacklisted)) {
			foreach ($secure_access_webinar_blacklisted as $blacklisted) {
				...
				if (false !== strpos($email, $blacklisted)) {
					self::error_response(['message' => __('Error', 'wp2leads_itm_mid') . ': ' . __('You are not authorized to register for the event.', 'webinarignition')]);
				}
			}
		}
	...
	public static function load_chat_messages() {
		if (empty($_POST['app_id']) || empty($_POST['email'])) {
			$params = array(
				'message' => __('Error', 'wp2leads_itm_mid') . ': ' . __('Cheating, huh!!!.', 'webinarignition'),
				'reload' => 0,
			);
	...
	public static function dev_remove_license() {
		...
		$params = array(
			'message' => __('Success', 'wp2leads_itm_mid') . ': ' . __('Key deleted.', 'webinarignition'),
			'reload' => 1,
		);
	...
	public static function dev_add_license() {
		...
		$params = array(
			'message' => __('Success', 'wp2leads_itm_mid') . ': ' . __('License activated.', 'webinarignition'),
			'reload' => 1,
		);
	...
	public static function deactivate_freemius() {
		...
		$params = array(
			'message' => __('Hooray!!!', 'wp2leads_itm_mid') . ' ' . __("Let's see what we have for you!!!", 'webinarignition'),
			'reload' => 1,
		);
